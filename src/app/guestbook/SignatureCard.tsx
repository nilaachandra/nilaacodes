      ...
      {posts.map((post) => (
        <li key={post.id} className="flex col-span-2 lg:col-span-1 w-full">
          <div className="rounded-lg w-full flex flex-col leading-none justify-between h-full bg-zinc-300  p-4">
            <p className="leading-none mb-2 text-sm">{post.message}</p>
            <p className="leading-none font-semibold text-xl italic font-instrumentSerif text-blue-800">
              By {post.name || "Unknown"}
            </p>
